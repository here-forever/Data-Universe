...
- `POST /api/insights/{dataset_id}/explore`
- `POST /api/insights/{dataset_id}/ask`
- `POST /api/insights/llm/check`
- `POST /api/insights/{dataset_id}/advanced`
- `GET|POST|PATCH|DELETE /api/stories...`
- `WS /api/collaboration/{dataset_id}/ws`
...
`POST /api/insights/llm/check` 可验证用户提供的临时模型配置，`POST /api/insights/{dataset_id}/ask` 也接受同结构的可选 `llm` 字段。临时 API Key 只用于当次上游请求，不写入分析历史。系统只向模型发送聚合画像与探索结果，不发送原始数据行。
